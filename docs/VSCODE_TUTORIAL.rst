...
.. note::
   ...
   このチュートリアルは、VS Codeで開発用コンテナ（devcontainer）を開いた後から開始します。
   VS Codeの起動方法と開発用コンテナを開くまでの手順は、利用環境に合わせて
   この文書の前へ追記してください。以降、開発用コンテナを単に「開発環境」と記載します。
...
* PE9とPE8は、互いに反対の動作をするPWM出力です。出力割合が0%または100%でも、片側がHighになる場合があります。
* 実際の電力回路では、異常時にPWMを強制停止する回路、過電流保護、上下同時ON防止等が必要です。
...
PC側のEthernet接続には、例としてIPアドレス ``192.168.100.1``、
サブネットマスク ``255.255.255.0`` を設定します。基板のIPアドレスは ``192.168.100.2`` です。
...
ファイルを保存したら、プログラム作成・書き込み用の欄で、再度ビルドと書き込みを行います。
...
A0へ3.3 Vを超える電圧を入力してはいけません。付属のフィルタ基板を指定どおりに接続してください。
...
収束することを確認します。応答が不安定な場合は直ちに ``OFF`` を入力してください。
...
* ``OFF`` でPWMが停止する
* ``fault=0x00000000`` と表示され、異常が検出されていない
...
#. 異常時にPWMを強制停止する回路、過電流保護、起動・停止手順を実装する
